...
result=$(curl -k -H "Content-Type: application/json"  -X POST -d "$(cat $DIR/test_data/tilstand_opret.json)" $HOST_URL/tilstand/tilstand)

uuid=$(expr "$result" : '.*"uuid": "\([^"]*\)"')

if [ ! -z $uuid ]
then
    echo "Oprettet tilstand: $uuid"
else
    echo "Opret tilstand fejlet: $result" 
    exit
fi
...
echo ""
echo "Importing tilstand" 

import_uuid=$(uuidgen)

curl -k --write-out %{http_code} --output /tmp/tilstand_opret.txt -sH "Content-Type: application/json"  -X PUT -d "$(cat $DIR/test_data/tilstand_opret.json)" $HOST_URL/tilstand/tilstand/$import_uuid 

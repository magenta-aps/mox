...
def read_config(config_path):
    try:
        ...
        logger.critical("%s: %r", strerror=err.strerror, filename=err.filename)
        sys.exit(5)
    try:
...
def update_config(configuration, new_settings):
    ...
    for key in new_settings:
        if key in configuration:
            ...
            logger.warning("Invalid key in config: %s", key=key)
...
if system_config_path:
    logger.info("Reading system config from %s", system_config_path=system_config_path)
    update_config(config, read_config(system_config_path))
if user_config_path:
    logger.info("Reading user config from %s", user_config_path=user_config_path)

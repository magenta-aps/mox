...
class TestDBObjectFunctions(TestCase):
    ...
    def test_create_or_import_object_executes_sql(self,
        ...
        # Arrange
        sql = "sqlstring"
        mock_jinja_env.get_template.return_value.render.return_value = sql
        ...
        self.assertEqual(sql, actual_sql)

    @patch("oio_rest.db.get_connection")
    @patch("oio_rest.db.jinja_env")
    def test_create_or_import_object_returns_cursor_result(self,
                                                           mock_jinja_env,
                                                           mock_get_conn):
        # type: (MagicMock, MagicMock) -> None
        # Arrange
        mock_get_conn.return_value.cursor.return_value = cursor = MagicMock()
        expected_result = "this is a result"

        cursor.fetchone.return_value = [expected_result]

        # Act
        actual_result = db.create_or_import_object("classname", "note", {})
    ...
    @patch("oio_rest.db.get_connection")
    @patch("oio_rest.db.jinja_env")
    def test_delete_object_returns_cursor_result(self,
                                                 mock_jinja_env,
                                                 mock_get_conn):
        # type: (MagicMock, MagicMock) -> None
        # Arrange
        ...
        # Act
        actual_result = db.delete_object("classname", {}, "note", "uuid")
    ...
    @patch("oio_rest.db.get_connection")
    @patch("oio_rest.db.jinja_env")
    def test_passivate_object_returns_cursor_result(self,
                                                    mock_jinja_env,
                                                    mock_get_conn):
        # type: (MagicMock, MagicMock) -> None
        # Arrange
        ...
        # Act
        actual_result = db.passivate_object("classname", {}, "note", "uuid")
    ...
    @patch("oio_rest.db.get_connection")
    @patch("oio_rest.db.jinja_env")
    def test_update_object_executes_sql(self,
                                        mock_jinja_env,
                                        mock_get_conn):
        # type: (MagicMock, MagicMock) -> None
        # Arrange
    ...
    @patch("oio_rest.db.get_connection")
    @patch("oio_rest.db.jinja_env")
    def test_list_objects_returns_cursor_result(self,
                                                mock_jinja_env,
                                                mock_get_conn):
        # type: (MagicMock, MagicMock) -> None
        # Arrange
        ...
        # Act
        actual_result = db.list_objects("classname", ["uuid"], "virkning_fra",
                                        "virkning_til",
    ...
    @patch("oio_rest.db.get_connection")
    @patch("oio_rest.db.jinja_env")
    def test_list_objects_executes_sql(self,
                                       mock_jinja_env,
                                       mock_get_conn):
        # type: (MagicMock, MagicMock) -> None
        # Arrange
    ...
    def test_list_objects_raises_on_no_results(self,
        ...
        with self.assertRaises(NotFoundException):
            actual_result = db.list_objects("classname", ["uuid"],
    ...
    @patch("oio_rest.db.get_connection")
    @patch("oio_rest.db.jinja_env")
    def test_search_objects_returns_cursor_result(self,
                                                  mock_jinja_env,
                                                  mock_get_conn):
        # type: (MagicMock, MagicMock) -> None
        # Arrange
        ...
        # Act
        actual_result = db.search_objects("classname", "uuid", {})
    ...
    @patch("oio_rest.db.get_connection")
    @patch("oio_rest.db.jinja_env")
    def test_search_objects_executes_sql(self,
                                         mock_jinja_env,
                                         mock_get_conn):
        # type: (MagicMock, MagicMock) -> None
        # Arrange
    ...
    @patch("oio_rest.db.get_connection")
    @patch("oio_rest.db.jinja_env")
    def test_object_exists_returns_cursor_result(self,
                                                 mock_jinja_env,
                                                 mock_get_conn):
        # type: (MagicMock, MagicMock) -> None
        # Arrange
        ...
        # Act
        actual_result = db.object_exists("classname", "uuid")

        # Assert
        self.assertEqual(expected_result, actual_result)

    @patch("oio_rest.db.get_connection")
    def test_object_exists_executes_sql_and_uuid(self,
                                                 mock_get_conn):
        # type: (MagicMock) -> None
        # Arrange
        classname = "classname"
        ...
        cursor.execute.assert_called()
        actual_sql = cursor.execute.call_args[0][0]
        actual_uuid = cursor.execute.call_args[0][1]
        self.assertEqual(expected_sql, actual_sql)
        self.assertEqual(expected_uuid, actual_uuid)

    @patch("oio_rest.db.get_connection")
    @patch("oio_rest.db.jinja_env")
    def test_get_document_returns_cursor_result(self,
                                                mock_jinja_env,
                                                mock_get_conn):
        # type: (MagicMock, MagicMock) -> None
        # Arrange
        ...
        # Act
        actual_result = db.get_document_from_content_url("content_url")

        # Assert
        self.assertEqual(expected_result, actual_result)

    @patch("oio_rest.db.get_connection")
    def test_get_document_executes_content_url(self,
                                               mock_get_conn):
        # type: (MagicMock) -> None
        # Arrange
        url = "content_url"
        ...
        # Act
        db.get_document_from_content_url(url)

        # Assert
        cursor.execute.assert_called()
        actual_url = cursor.execute.call_args[0][1]
...
class TestDBGeneralSQL(TestCase):
    ...
    @patch('oio_rest.db.jinja_env.get_template')
    def test_sql_get_registration(self, mock_jinja_env):
        # type: (MagicMock) -> None
        # Arrange
        mock_jinja_env.return_value = template = MagicMock()
        template.render.return_value = sql = MagicMock()
        ...
        # Act
        result = db.sql_get_registration(class_name, time_period,
                                         life_cycle_code,

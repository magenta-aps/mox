...
class TestBooleanAttribute(ExtDBTestCase):
    db_structure_extensions = {
        "facet": {
            "attributter": {"udvidelser": ["primær"]},
            "attributter_metadata": {"udvidelser": {"primær": {"type": "boolean"}}},
        }
    }
    ...
    def test_create_and_search(self):
        ...
        self.assertQueryResponse(
            "/klassifikation/facet", [primary, secondary, unspecified, missing], bvn="%"
        )

        self.assertQueryResponse("/klassifikation/facet", [secondary], primær="0")

        self.assertQueryResponse("/klassifikation/facet", [primary], primær="1")

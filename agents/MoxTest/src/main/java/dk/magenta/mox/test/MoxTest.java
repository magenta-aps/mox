...
public class MoxTest extends MoxAgent {
    ...
    private void testDokument() {
        if (this.sender != null) {
            try {
                UUID dokument = this.testDokumentOpret();
                if (dokument != null) {
                    try {
                        this.testDokumentRead(dokument);
                        this.testDokumentSearch();
                        //this.testDokumentList(dokument);
                        this.testDokumentUpdate(dokument);
                        this.testDokumentPassivate(dokument);
                    } catch (TestException e) {
                    }
                    this.testDokumentDelete(dokument);
                }
            } catch (TestException e) {
                e.printStackTrace();
            }
    ...
    private UUID testDokumentOpret() throws TestException {
        String response = null;
        try {
            printDivider();
            System.out.println("Creating dokument");
            Headers headers = this.getBaseHeaders();
            headers.put(Message.HEADER_OBJECTTYPE, "dokument");
            headers.put(Message.HEADER_OPERATION, CreateDocumentMessage.OPERATION);
            JSONObject payload = getJSONObjectFromFilename("data/dokument/create.json");
            Message message = CreateDocumentMessage.parse(headers, payload);
            response = this.sender.send(message, true).get(30, TimeUnit.SECONDS);
            JSONObject object = new JSONObject(response);
            UUID uuid = UUID.fromString(object.getString("uuid"));
            System.out.println("Dokument created, uuid: "+uuid.toString());
            System.out.println("Create succeeded");
            return uuid;
        } catch (JSONException e) {
            System.out.print(response);
    ...
    private void testDokumentRead(UUID uuid) throws TestException {
        String response = null;
        try {
            printDivider();
            System.out.println("Reading dokument, uuid: "+uuid.toString());
            Message message = new ReadDocumentMessage(this.getAuthToken(), "dokument", uuid);
            response = this.sender.send(message, true).get(30, TimeUnit.SECONDS);
            System.out.println(response);
            JSONObject object = new JSONObject(response);
            JSONObject item = object.getJSONArray(uuid.toString()).getJSONObject(0);

            JSONObject expected = getJSONObjectFromFilename("data/dokument/read_response.json");
    ...
    private List<UUID> testDokumentSearch() throws TestException {
        String response = null;
        try {
            printDivider();
            System.out.println("Searching for dokument");
            ParameterMap<String, String> query = new ParameterMap<>();
            query.populateFromJSON(getJSONObjectFromFilename("data/dokument/search.json"));
            Message message = new SearchDocumentMessage(this.getAuthToken(), "dokument", query);
            response = this.sender.send(message, true).get(30, TimeUnit.SECONDS);
            ArrayList<UUID> results = new ArrayList<>();
            JSONObject object = new JSONObject(response);
            JSONArray array = object.getJSONArray("results");
...
/*
    private void testDokumentList(UUID uuid) throws TestException {
        String response = null;
        try {
            printDivider();
            System.out.println("Listing dokuments");
            Message message = new ListDocumentMessage(this.getAuthToken(), "dokument", uuid);
            response = this.sender.send(message, true).get(30, TimeUnit.SECONDS);
            JSONObject object = new JSONObject(response);
            JSONArray array = object.getJSONArray("results");
            try {
    ...
    private void testDokumentUpdate(UUID uuid) throws TestException {
        String response = null;
        try {
            printDivider();
            System.out.println("Updating dokument, uuid: "+uuid.toString());
            Message message = new UpdateDocumentMessage(this.getAuthToken(), "dokument", uuid, getJSONObjectFromFilename("data/dokument/update.json"));
            response = this.sender.send(message, true).get(30, TimeUnit.SECONDS);
            JSONObject object = new JSONObject(response);
            UUID result = UUID.fromString(object.getString("uuid"));
    ...
    private void testDokumentPassivate(UUID uuid) throws TestException {
        String response = null;
        try {
            printDivider();
            System.out.println("Passivating dokument, uuid: "+uuid.toString());
            Message message = new PassivateDocumentMessage(this.getAuthToken(), "dokument", uuid, "Passivate, please");
            response = this.sender.send(message, true).get(30, TimeUnit.SECONDS);
            JSONObject object = new JSONObject(response);
            UUID result = UUID.fromString(object.getString("uuid"));
            if (uuid.compareTo(result) == 0) {
    ...
    private void testDokumentDelete(UUID uuid) throws TestException {
        String response = null;
        try {
            printDivider();
            System.out.println("Deleting dokument, uuid: "+uuid.toString());
            Message message = new DeleteDocumentMessage(this.getAuthToken(), "dokument", uuid, "Delete, please");
            response = this.sender.send(message, true).get(30, TimeUnit.SECONDS);
            JSONObject object = new JSONObject(response);
            UUID result = UUID.fromString(object.getString("uuid"));
            if (uuid.compareTo(result) == 0) {
